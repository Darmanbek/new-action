...
} from "src/services/acceptances"
import { completeColor, completeIcon, phoneFormatter } from "src/utils"

export const useColumnsAcceptances = () => {
	...
	const columns: ColumnsType<TAcceptance> = [
		...
		},
		{
			...
			dataIndex: "group",
			key: "group",
			render: (group: TAcceptance["group"]) => <Link to={`/groups/${group.id}`}>{group.name}</Link>
		},
		{
			ellipsis: true,
			align: "center",
			...
			render: (is_acceptance: boolean) => (
				<UiTag icon={completeIcon(is_acceptance)} color={completeColor(is_acceptance)}>
