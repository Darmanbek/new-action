...
const UiInputChat: FC<InputProps & RefAttributes<InputRef> & UiInputChatProps> = (props) => {
	...
	const onHandleMessage = useCallback((message: string) => {
		if (onChange) {
			onChange(message);
		}
	}, [onChange]);

	const onEmojiMessage = useCallback((emoji: EmojiClickData) => {
				...
				prefix={
					<Popover
						content={
							<EmojiPicker
								theme={Theme.LIGHT}
								onEmojiClick={onEmojiMessage}
							/>
						}
						trigger={"click"}
					>
						<BsEmojiSmile onClick={(e) => e.stopPropagation()} color={"royalblue"} style={{ fontSize: 21 }} />
					</Popover>
				}
				suffix={
					loading ? (
