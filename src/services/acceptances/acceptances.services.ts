...
const axiosGetAcceptance = async (params: TGetParams): Promise<TResponse<TAcceptance>> => {
...
const axiosDeleteAcceptance = async (id?: number | string): Promise<TResponseSingleData<void>> => {
	const response = await api.delete(`/admin/acceptances/${id}`)
	return response.data
}

export { axiosGetAcceptance, axiosCreateAcceptance, axiosEditAcceptance, axiosDeleteAcceptance }
